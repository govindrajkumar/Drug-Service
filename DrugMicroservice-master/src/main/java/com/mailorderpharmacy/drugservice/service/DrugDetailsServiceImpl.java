...
public class DrugDetailsServiceImpl implements DrugDetailsService {
	...
	@Override
	public DrugDetails getDrugById(final String id, final String token)
	...
	@Override
	public DrugDetails getDrugByName(final String name, final String token)
			throws InvalidTokenException, DrugNotFoundException {
		if (((TokenValid) this.authFeign.getValidity(token).getBody()).isValid()) {
			try {
				return this.drugRepo.findBydrugName(name).get();
			} catch (Exception DrugExcp) {
	...
	@Override
	public Stock getDispatchableDrugStock(final String id, final String location, final String token)
	...
	@Override
	public ResponseEntity<SuccessResponse> updateQuantity(final String drugName, final String location,
			final int quantity, final String token)
		...
		final List<DrugLocationDetails> dummy = details.getDruglocationQuantities().stream()
		...
		if (dummy.get(0).getQuantity() >= quantity) {
			...
			return (ResponseEntity<SuccessResponse>) new ResponseEntity(

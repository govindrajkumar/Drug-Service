...
class DrugControllerTest {
	...
	void testGetDrugById() throws Exception {
		...
		final MvcResult result = this.mockMvc.perform((RequestBuilder) MockMvcRequestBuilders
				.get("/searchDrugsById/D1", new Object[0]).header("Authorization", new Object[] { "Bearer Token" }))
				.andReturn();
	...
	void testGetDrugByName() throws Exception {
		this.list.add(new DrugLocationDetails("D1", "Chennai", 30, null));
		final DrugDetails expected = new DrugDetails("D1", "Paracetamol", "manu1", new Date(), new Date(), this.list);
		...
		final MvcResult result = this.mockMvc
				.perform((RequestBuilder) MockMvcRequestBuilders.get("/searchDrugsByName/Paracetamol", new Object[0])
	...
	void testDispatchableDrugStock() throws Exception {
		final Stock expectedStock = new Stock("D1", "Paracetamol", new Date(), 30);
		final TokenValid tokenValid = new TokenValid("uid", "uname", true);
		@SuppressWarnings("unchecked")
		final ResponseEntity<TokenValid> response = (ResponseEntity<TokenValid>) new ResponseEntity((Object) tokenValid,
				HttpStatus.OK);
		Mockito.when((Object) this.authFeign.getValidity("Bearer Token")).thenReturn((Object) response);
		Mockito.when((Object) this.drugDetailsService.getDispatchableDrugStock("D1", "Chennai", "Bearer token"))
				.thenReturn((Object) expectedStock);
		Mockito.when((Object) this.drugController.getDispatchableDrugStock("Bearer token", "D1", "Chennai"))
				.thenReturn((Object) expectedStock);
		final MvcResult result = this.mockMvc.perform(
				(RequestBuilder) MockMvcRequestBuilders.get("/getDispatchableDrugStock/D1/Hyderabad", new Object[0])
						.header("Authorization", new Object[] { "Bearer Token" }))
				.andReturn();
		final String actualValue = result.getResponse().getContentAsString();
		final String expectedValue = "{\"drugId\":\"D1\",\"drugName\":\"Paracetamol\",\"expiryDate\":\"2024-11-21T18:30:00.000+00:00\",\"stock\":30}";
		Assertions.assertEquals((Object) expectedValue, (Object) actualValue);
	}
	...
	void testupdateQuantity() throws Exception {
		final TokenValid tokenValid = new TokenValid("uid", "uname", true);
		final ObjectMapper objectMapper = new ObjectMapper();
		final ResponseEntity<SuccessResponse> expectedValue = (ResponseEntity<SuccessResponse>) new ResponseEntity(
				(Object) new SuccessResponse("Refill done successfully"), HttpStatus.OK);
		@SuppressWarnings("unchecked")
		final ResponseEntity<TokenValid> response = (ResponseEntity<TokenValid>) new ResponseEntity((Object) tokenValid,
				HttpStatus.OK);
		Mockito.when((Object) this.authFeign.getValidity("Bearer Token")).thenReturn((Object) response);
		Mockito.when((Object) this.drugDetailsService.updateQuantity("D1", "Chennai", 1, "Bearer token"))
				.thenReturn((Object) expectedValue);
		Mockito.when((Object) this.drugController.updateQuantity("D1", "Chennai", "Bearer token", 1))
				.thenReturn((Object) expectedValue);
		final MvcResult result = this.mockMvc.perform((RequestBuilder) MockMvcRequestBuilders
				.get("/updateDispatchableDrugStock/D1/Hyderabad/1", new Object[0])
				.header("Authorization", new Object[] { "Bearer Token" })).andReturn();
	...
	void testgetAllDrugs() throws Exception {
		...
		final MvcResult result = this.mockMvc.perform((RequestBuilder) MockMvcRequestBuilders
				.get("/getAllDrugs", new Object[0]).header("Authorization", new Object[] { "Bearer Token" }))
				.andReturn();
		final String actualValue = result.getResponse().getContentAsString();
